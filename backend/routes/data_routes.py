...
# Load fake CCTV analytics (generated by generate_fake_analytics.py)
_fake_cctv_path = DATA_DIR / "processed" / "cctv" / "fake_analytics.json"
_fake_cctv: dict = {}
if _fake_cctv_path.exists():
    with open(_fake_cctv_path) as _f:
        _fake_cctv = json.load(_f)
...
async def get_cctv_timeseries(neighborhood: str):
    ...
    entry = _fake_cctv.get(neighborhood)
    if not entry:
...
async def get_neighborhood(name: str):
    ...
    # CCTV fake analytics
    cctv_entry = _fake_cctv.get(name, {})
