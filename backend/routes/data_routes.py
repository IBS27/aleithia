...
# Load synthetic CCTV analytics (generated by generate_synthetic_analytics.py)
_synthetic_cctv_path = DATA_DIR / "processed" / "cctv" / "synthetic_analytics.json"
_synthetic_cctv: dict = {}
if _synthetic_cctv_path.exists():
    with open(_synthetic_cctv_path) as _f:
        _synthetic_cctv = json.load(_f)
...
async def get_cctv_timeseries(neighborhood: str):
    ...
    entry = _synthetic_cctv.get(neighborhood)
    if not entry:
...
async def get_neighborhood(name: str):
    ...
    cctv_entry = _synthetic_cctv.get(name, {})
